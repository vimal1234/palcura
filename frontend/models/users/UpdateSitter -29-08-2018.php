...
class UpdateSitter extends Users {
    ...
    public function rules() {
        return [
           ...
           // [['firstname'], 'string', 'max' => 60, 'on' => 'update'],                        
            [['zip_code','country','region','city', 'services', 'day_price', 'pitch','pet_parent_type_sr'], 'string', 'max' => 60, 'on' => 'update'],                        
	...
	*/
    public function updateRecords($id,$mediaArr) 
	{
        /* if (!$this->validate()) {
            return null;
        } */
						...
						if(isset($UserServices) && !empty($UserServices)) {
							...
							for($i = 0; $i < count($this->services); $i++) {
								//$data1 					= new UserServices();
									...
									$data1->service_id	= (isset($this->services[$i]) ? $this->services[$i] : 0);									$data1->pet_type_id = $this->pet_parent_type_sr[$i];							
										if($data1->save()) {
											$uscount=1;
										}
									}else{
