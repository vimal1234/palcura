    ...
    public function behaviors() {
        return [
            'access' => [
                ...
                'rules' => [
                    [
                        'actions' => ['index','update','myprofile','settings','dashboard','becomeasitter','becomeanowner','view-user-details','activitylog','updateactivity','newdashboardlist','addavailability','switch-account1','switch-account','newimagelist','removeservice','removepet','testview','bookingreminders','becomeaborrower','updatesubscription','bookingprice'],
                        'allow' => true,
                        'roles' => ['@'],
                    ],
                ],
    ...
    public function actionSettings() {
		...
		if(Yii::$app->request->isPost){ 
					...
					if(!$updateUser->validate()){ 	
						$errors1 = $updateUser->errors;
						foreach($errors1 as $key=>$data){
							$error1.='<i class="fa fa-warning"></i> '.$data[0].'<br>';
						}
					}	
					$model->profile_image 	= '';
					$model->emai=$userpost['email'];
					...
					if($model->load(Yii::$app->request->post()) && $model->updateRecords($id,$data=null,$service_data=0)){
								
							
						#THIS CODE FOR UPLOAD ID OR DOCUMENT IMAGE 
						if(isset($_FILES['Uploads']['name']['upload_documents']) && $_FILES['Uploads']['name']['upload_documents'][0]!=''){
							...
							if ($modelImageUploadB->upload_documents && $uploadedFileNameArray = $modelImageUploadB->uploadB()){
								...
								$model->updateRecords($id,$documentsArr,$service_data=0);
						}else{
								$Identity_documents	=	Yii::$app->commonmethod->getUserDocuments($id,ID_DOCUMENTS);
							if(count($Identity_documents)<1) {
								
									$error1.='<i class="fa fa-warning"></i> Driver’s License or Photo ID required.<br>';
													
							}
						}
						
						#THIS CODE DEVLOPED FOR HOME IMAGE [ HOME IMAGE ATLESAST TWO REQUIRED ]
						if($modelImageUploadD->load(Yii::$app->request->post()) && $modelImageUploadD->validate()){
							...
							$documentsArr = array();
								...
								#THIS CODE DEVLOPED FOR ADDITIONAL IMAGE 
								...
								if (isset($model->profile_completed) AND !empty($model->profile_completed)){
									...
									$emailSitter=( isset(Yii::$app->request->post()['UpdateSitter']['email']) ? Yii::$app->request->post()['UpdateSitter']['email'] : '' );
									$sitterName = ( isset(Yii::$app->request->post()['UpdateSitter']['firstname']) ? Yii::$app->request->post()['UpdateSitter']['firstname'] : '' );
									$lastName = ( isset(Yii::$app->request->post()['UpdateSitter']['lastname']) ? Yii::$app->request->post()['UpdateSitter']['lastname'] : '' );
									
							
									#DEFINE CREDENTIAL FOR MAILCHIMP API
										$list_id = '10f7c53647';
										$api_key = 'f1f9682348cebeff5f49e9ca83fa1b38-us18';
										$data_center = substr($api_key,strpos($api_key,'-')+1);
										$url = 'https://'. $data_center .'.api.mailchimp.com/3.0/lists/'. $list_id .'/members/'. md5(strtolower($emailSitter));
									#HIT MAILCHIMP API USING CURL
										$ch = curl_init($url);
									...
									#SEND THANKING EMAIL TO SITTER FOR COMPLITING HIS PROFIL
									...
									//	$this->sendprofilecompletionemail('admin',$email="",$sitterName,'sitter');
										if($model->load(Yii::$app->request->post()) && $model->updateRecords($id,$documentsArr,$service_data=1)) {
								...
								$ok= $model->updateRecords($id,$documentsArr,$service_data=1);
								echo "hhhh".$ok;
								exit;
								if($model->load(Yii::$app->request->post()) && $ok){
									...
									$user_profile_images	=	Yii::$app->commonmethod->getUserProfilepics($id);
								
									if($user_profile_images['0']['profile_image']==''){
										$error1.='<i class="fa fa-warning"></i> Please upload your profile image.<br>';
									}
									$user_home_images	=	Yii::$app->commonmethod->getUserDocuments($id,HOME_IMAGES);
									if(count($user_home_images)=='0'){
										$error1.='<i class="fa fa-warning"></i> Minimum two inside house pictures are required.<br>';
									
									}else if(count($user_home_images)=='1') {
										$error1.='<i class="fa fa-warning"></i> Please upload 1 additional in-house picture.<br>';
			
									}
							
									if($error1=='') {
										
									   $model->update_profile_id($id);
										Yii::$app->session->setFlash('item', '<i class="fa fa-check"></i> Your profile has been updated successfully!');
									}else {
										Yii::$app->session->setFlash('item', '<i class="fa fa-check"></i> Your profile is saved! Please fill in following pending items:<br>'.$error1);
									}
						...
						if($error1=='') {
							 ...
							 Yii::$app->session->setFlash('item', '<i class="fa fa-check"></i> Your profile has been updated successfully!');
								...
								if(isset($_FILES['Uploads']['name']['upload_documents']) && $_FILES['Uploads']['name']['upload_documents'][0]!=''){
									...
									if(count($Identity_documents)<1) {
										$errors1= $modelImageUploadB->errors;
										$error1.='<i class="fa fa-warning"></i> Driver’s License or Photo ID required.<br>';
    ...
    public function actionBecomeasitter() {
		...
		if (isset($userpost) && !empty($userpost) && $modelImageUploadD->load(Yii::$app->request->post()) && $modelImageUploadD->validate()) {
			...
			if (Yii::$app->request->isPost) {
				...
				//$model->city
				$model->address = Yii::$app->request->post()['UpdateSitter']['address'];
				$model->pet_service_id = Yii::$app->request->post()['UpdateSitter']['pet_service_id'];
				$model->pet_weight_limit = Yii::$app->request->post()['UpdateSitter']['pet_weight_limit'];
...
'renterprice' => $renterprice
	...
	public function actionSwitchAccount1($user_type=0) {
		$session 	= Yii::$app->session;
		$user_type 	= Yii::$app->request->get('id');
		$attributes = Yii::$app->user->identity->getattributes();
		$cr_user_type = (isset($attributes['user_type']) && $attributes['user_type'] > 0 ? $attributes['user_type'] : 0);
		
		
		if($cr_user_type == SITTER) {
			$id = Yii::$app->user->getId();
			
			
			#CHECK USER REQUEST FOR SWITCH ACCOUNT LIKE 1=OWNER 3=SITTER  
			if($user_type == OWNER) {
				$updateUser = new UpdateOwner();
				$attributes['user_type'] = $updateUser->sitter_to_owner($id);
			} else if($user_type == BORROWER) {
				$updateUser = new UpdateRenter();	
				$attributes['user_type'] = $updateUser->sitter_to_borrower($id);
			}	
