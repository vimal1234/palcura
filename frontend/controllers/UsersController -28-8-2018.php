...
if (isset($userpost) && !empty($userpost) && $modelImageUploadK->load(Yii::$app->request->post()) && $modelImageUploadK->validate()) { 
		...
		{ 
			...
			if($model->load(Yii::$app->request->post()) && $model->updateRecords($id,$data=null)) 
			{
				...
				echo $modelImageUploadD->validate();
				exit; */
				
				 
				if(isset($userpost) && !empty($userpost) && (($modelImageUploadD->load(Yii::$app->request->post()) && $modelImageUploadD->validate()) || (isset($_FILES['Uploads']['name']['upload_documents']) && $_FILES['Uploads']['name']['upload_documents'] !=''))) 
				{
						...
						if($model->load(Yii::$app->request->post()) && $model->updateRecords($id,$documentsArr)) 
						{
							...
							if($error1=='') {
								//$//connection=Yii::app()->db;
								 ...
								 //$connection->createCommand('UPDATE user SET profile_completed="1" WHERE id='.$id)->execute();
								$model->update_profile_id($id);
								
								// Yii::app()->db->createCommand()->update('user',array('profile_completed'=>'1'),'id=:id',array(':id'=>360));
								Yii::$app->session->setFlash('item', '<i class="fa fa-check"></i> Your profile has been updated successfully!');
							} else {
