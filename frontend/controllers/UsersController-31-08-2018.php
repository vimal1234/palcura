...
if (isset($userpost) && !empty($userpost) && $modelImageUploadK->load(Yii::$app->request->post()) && $modelImageUploadK->validate()) { 
		...
		{ 
			...
			{ 	//echo "tes";
				...
				//$this->addError('firstname', Yii::t('yii', 'Please select a file.'));	
			}	
			...
			if($model->load(Yii::$app->request->post()) && $model->updateRecords($id,$data=null,$service_data=1)) 
			{
				
			
				if(isset($_FILES['Uploads']['name']['upload_documents']) && $_FILES['Uploads']['name']['upload_documents'][0]!=''){
					//echo $_FILES['Uploads']['name']['upload_documents'];
				...
				if($modelImageUploadD->load(Yii::$app->request->post()) && $modelImageUploadD->validate()) 
				{
					...
					else
					{
						...
						//	$this->sendprofilecompletionemail('admin',$email="",$sitterName,'sitter');
								...
								if($model->load(Yii::$app->request->post()) && $model->updateRecords($id,$documentsArr,$service_data=0)) {
					...
					if($user_profile_images['0']['profile_image']==''){
						$error1.='<i class="fa fa-warning"></i> Please upload your profile image.<br>';
							...
							if(count($user_home_images)=='0')
							{
								...
								$error1.='<i class="fa fa-warning"></i> Still one image required for house pictures.<br>';
							}
							...
							//print_r($errors1); exit;
							
							//Yii::$app->session->setFlash('item', $error1);
