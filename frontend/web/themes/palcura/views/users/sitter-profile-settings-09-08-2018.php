...
//$ajaxSUrl	= 'http://192.168.1.250:8083/Palcura/ajax/updatestates';
//$ajaxCUrl	= 'http://192.168.1.250:8083/Palcura/ajax/updatecities';
...
<section class="contentArea sitter-profile-settings">
	<div class="container">
	    ...
	    <?php  } */?> 
						...
						<div class="row-block settings-section">	
							...
							<div class="col-md-4 col-sm-6 col-xs-12 image-upload">
	...
	<label for="userprofilepictureupload-profile_image">Upload Your Primary Profile Image<span> * </span></label>
	<input type="hidden" name="UserProfilePictureUpload[profile_image]" value="">
	<input type="file" id="userprofilepictureupload-profile_image" class="form-control imageBrowser" style="display:none" name="UserProfilePictureUpload[profile_image]">
	<div class="brbtnmain">					
		<span class="brbtn" onclick="document.getElementById('userprofilepictureupload-profile_image').click()">Browse</span>
		<span id="cngname" class="brbtnno">No files selected</span>
		<script>
		$('document').ready(function(){
			$('#userprofilepictureupload-profile_image').change(function() {
			  var file = $('#userprofilepictureupload-profile_image')[0].files[0].name;
			  $('#cngname').html(file);
			});
		});		
		</script>
							...
							<div class="col-md-8 col-sm-6 col-xs-12 image-upload">
								<?php
							...
							<div class="col-md-12 col-sm-6 col-xs-12 image-upload">
							...
							<?php
								...
								?>
		<label for="uploads-upload_images">Additional images (optional)<br>(Add pictures that will help pet owners get to know you and your love for pets.)</label>
		<input type="hidden" name="Uploads[upload_images][]" value="">
		<input type="file" id="uploads-upload_images" style="display:none" class="form-control imageBrowser" name="Uploads[upload_images][]" multiple="" accept="image/*">								
		<div class="brbtnmain">					
			<span class="brbtn" onclick="document.getElementById('uploads-upload_images').click()">Browse</span>
			<span id="cngname1" class="brbtnno">No files selected</span>
			<script>
			$('document').ready(function(){
				$('#uploads-upload_images').change(function() {
				  var file = $('#uploads-upload_images')[0].files[0].name;
				  $('#cngname1').html(file);
				});
			});		
			</script>
							...
							<div class="row-block">
								<div class="col-md-6 col-sm-6 col-xs-12 image-upload">
									<?php
									#### ID documents
									...
									?>
		<label for="uploads-upload_documents">Upload a picture of your Driver’s License or Photo ID<span> * </span></label>
		<input type="hidden" name="Uploads[upload_documents][]" value="">
		<input type="file" id="uploads-upload_documents" style="display:none" class="form-control imageBrowser" name="Uploads[upload_documents][]" multiple="">
		<div class="brbtnmain">					
			<span class="brbtn" onclick="document.getElementById('uploads-upload_documents').click()">Browse</span>
			<span id="cngname2" class="brbtnno">No files selected</span>
			<script> 
			$('document').ready(function(){
				$('#uploads-upload_documents').change(function() {
				  var file = $('#uploads-upload_documents')[0].files[0].name;
				  $('#cngname2').html(file);
				});
			});		
			</script>
								...
								<!--input id="uploadFile" placeholder="Add files from My Computer"/-->
									...
									?>
		<label for="uploads-upload_home_images">Upload 2 or more images of the inside of your home.<span> * </span><br>(Rest assured these pictures will not be displayed to the owners)</label>
		<input type="hidden" name="Uploads[upload_home_images][]" value="">
		<input type="file" id="uploads-upload_home_images" style="display:none" class="form-control imageBrowser" name="Uploads[upload_home_images][]" multiple="" accept="image/*">									
		<div class="brbtnmain">					
			<span class="brbtn" onclick="document.getElementById('uploads-upload_home_images').click()">Browse</span>
			<span id="cngname3" class="brbtnno">No files selected</span>
			<script> 
			$('document').ready(function(){
				$('#uploads-upload_home_images').change(function() {
				  var file = $('#uploads-upload_home_images')[0].files[0].name;
				  $('#cngname3').html(file);
				});
			});		
			</script>
							...
							<div class="col-md-12 col-sm-12 col-xs-12">
								...
								<?php
									...
									'class' => "form-control",
									]])->textInput(['maxlength'=>150 , 'autofocus' => true])->label('PayPal Email <span> * </span><br>(Payment will be made to your paypal account after completion of service)');
								?>   
							</div>
						</div>		
...
<script>
	...
	$('#addmore_services').click(function() {
			...
			$('#addmoreblock').append('<div id="'+newname+'_outer" class="row-block spasaller"><div style="display:none;"><input id="updatesitter-pet_service_id" class="form-control" name="UpdateSitter[pet_service_id][]" value="0" type="hidden"></div><div class="row-block"><div class="col-md-6 col-sm-6 col-xs-12"><span class=" field-updatesitter-pet_parent_type_sr required"><div class="form-group"><label for="updatesitter-pet_parent_type_sr">Choose the type of pet you can care for <span class="required">*</span></label><select id="updatesitter-pet_parent_type_sr" class="form-control whiteBorder selctpettype" name="UpdateSitter[pet_parent_type_sr][]"><option value="1">Dog</option><option value="2">Cat</option><option value="3">Other</option></select><div class="col-lg-12"><div class="row"><p class="help-block help-block-error"></p></div></div> </div></span></div><div class="col-md-6 col-sm-6 col-xs-12 selectsearchoptions weightclass"><span class="field-updatesitter-pet_weight_limit"><div class="form-group"><label for="updatesitter-pet_weight_limit">Choose the sizes of pet you would accept <span class="required">*</span></label><select id="updatesitter-pet_weight_limit" class="form-control whiteBorder selectpicker" name="UpdateSitter[pet_weight_limit][][]" size="4" data-live-search="true" multiple>'+'<?php echo $petweightOptions ?>'+'</select></div><div class="col-lg-12"><div class="row"><p class="help-block help-block-error"></p></div></div></span></div></div><div class="col-md-6 col-sm-6 col-xs-12"><span class="field-updatesitter-services has-success"><div class="form-group"><label for="updatesitter-services">Services <span class="required">*</span></label><select id="updatesitter-services" class="form-control whiteBorder lblchang'+newname+'" name="UpdateSitter[services][]">'+'<?= $serviceOptions ?>'+'</select><div class="col-lg-12"><div class="row"><p class="help-block help-block-error"></p></div></div></div></span></div><div class="col-md-4 col-sm-6 col-xs-12"><span class=" field-updatesitter-day_price"><div class="form-group"><label for="updatesitter-day_price">Price <span>*</span><span id="lbl'+newname+'"></span></label><input id="updatesitter-day_price" class="form-control" name="UpdateSitter[day_price][]" maxlength="5" min="1" autofocus="" type="number"><div class="col-lg-12"><div class="row"><p class="help-block help-block-error"></p></div></div></div></span></div><div class="col-md-2 col-sm-6 col-xs-12"><a href="javascript:void(0)" class="orangeBtn remove_service_row" id="'+newname+'" >-</a></div></div>');
