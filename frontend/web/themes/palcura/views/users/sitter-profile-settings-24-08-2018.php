...
//$ajaxSUrl	= 'http://localhost/Palcura/ajax/updatestates';
//$ajaxCUrl	= 'http://localhost/Palcura/ajax/updatecities';
...
<script>
	$(document).ready(function() {
		/*
		$("#editUser-submit").click(function() {
			
			var pet_types=$("#updatesitter-pet_parent_type_sr-0").val();
			var numFiles = $("input:file")[3].files.length;
			if (numFiles <= 1) {
					alert('Upload 2 or more images of the inside of your home');
					return false;
			}
			var photoid = $("input:file")[2].files.length;
			if (photoid < 1) {
					alert('Upload a picture of your Driver’s License or Photo ID');
					return false;
			}
			var photoid = $("input:file")[0].files.length;
			if (photoid < 1) {
					alert('Upload Your Primary Profile Image');
					return false;
			}
			...
			if(pet_types == "1") {
				if($("#weightid0 button span.filter-option").html() == "Nothing selected") {
					alert("Choose the sizes of pet you would accept");
					return false;
				}
			}
