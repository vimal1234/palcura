...
{
	$(":text,:file,select,number,textarea").change(function ()
	{
		var fld1 = fld2 = fld3 = fld4 = fld5 = fld6 = fld7 = fld8 = fld9 = fld10 = fld11 = fld12 = fld13 = fld14 = fld15 = 0;
		$(":text,number,:file,select,textarea").each(function() 
		{
			if ($(this).attr("name") == 'UpdateSitter[firstname]') 
			{	
				if($('#updatesitter-firstname').val() == '') 
				{
					fld1 = 1;
				}
				else
			...
			if ($(this).attr("name") == 'UpdateSitter[lastname]') 
			{
				if($('#updatesitter-lastname').val() == '') 
				{
					fld2 = 1;
				}
				else
			...
			if ($(this).attr("name") == 'UpdateSitter[email]') 
			{
				if($('#updatesitter-email').val() == '') 
				{
					fld3 = 1;
				}
				else
			...
			if ($(this).attr("name") == 'UpdateSitter[address]') 
			{
				if($('#search_destination1').val() == '') 
				{
					fld4 = 1;
				}
				else
			...
			if ($(this).attr("name") == 'UpdateSitter[zip_code]') 
			{
				if($('#updatesitter-zip_code').val() == '') 
				{
					fld5 = 1;
				}
				else
			...
			if ($(this).attr("name") == 'UpdateSitter[country]') 
			{
				if($('#updatesitter-country').val() == '') {
					fld6 = 1;
				}
				else
				{
			...
			if ($(this).attr("name") == 'UpdateSitter[region]') 
			{
				if($('#updatesitter-region').val() == '') {
					fld7 = 1;
				}
				else
				{
			...
			if ($(this).attr("name") == 'UpdateSitter[city]') 
			{
				if($('#updatesitter-city').val() == '') {
					fld8 = 1;
				}
				else
				{
			...
			if ($(this).attr("name") == 'Uploads[upload_home_images][]') 
			{
				if(document.getElementById("uploads-upload_home_images").files.length < 2) 
				{
					fld9 = 1;
				}
				else
			...
			if ($(this).attr("name") == 'UpdateSitter[pitch]') 
			{
				if(document.getElementById("updatesitter-pitch")!=null || document.getElementById("updatesitter-pitch")!='') 
				{
					fld10 = 0;
				}
				else
			...
			if ($(this).attr("name") == 'UserProfilePictureUpload[profile_image]') 
			{			
				if(document.getElementById("userprofilepictureupload-profile_image").files.length > 0) 
				{
					fld11 = 0;
				}
				else
			...
			if ($(this).attr("name") == 'Uploads[upload_documents][]') 
			{
				if(document.getElementById("uploads-upload_documents").files.length >0) {
					fld12 = 0;
				}
				else
				{
			...
			if ($(this).attr("name") == 'UpdateSitter[pet_parent_type_sr][]' ) 
			{
			...
			if ($(this).attr("name") == 'UpdateSitter[pet_weight_limit][0][]') 
			{
				if($('.selectpicker').selectpicker('val')!=null) {
					fld14 = 0;
				}
			...
			if ($(this).attr("name") == 'UpdateSitter[day_price][]') 
			{
				if($('#updatesitter-day_price').val() == '') {
					fld15 = 1;
				}
				else
				{
			...
			//alert(fld1+' '+fld2+' '+fld3+' '+fld4+' '+fld5+' '+fld6+' '+fld7+' '+fld8+' '+fld9+' '+fld10+' '+fld11+' '+fld12+' '+fld13+' '+fld4+' '+fld15);
			if (fld1 == 0 && fld2 == 0 && fld3 == 0 && fld4 == 0 && fld5 == 0 && fld6 == 0 
			&& fld7 == 0 && fld8 == 0 && fld9 == 0 && fld10 == 0 && fld11 == 0 && fld12 == 0 && fld13 == 0 
			&& fld14 == 0 && fld15 == 0) 
			{
				$('#editUser-submit1').removeAttr('disabled');
				$('#editUser-submit1').addClass('orangeBtn');
				$('#editUser-submit1').removeClass('orangeBtn-disable');
				
			} 
			else 
			{
				$('#editUser-submit1').attr("disabled", true);
				$('#editUser-submit1').addClass('orangeBtn-disable');
				$('#editUser-submit1').removeClass('orangeBtn');	
			}
						...
						<div class="row-inner">
							...
							<div class="col-md-4 col-sm-6 col-xs-12">
								<?php
									...
									'class' => "form-control textfeild online",'id' => "search_destination1"
							...
							<div class="col-md-4 col-sm-6 col-xs-12">
								<?php
									...
									'class' => "form-control whiteBorder online",
						...
						<div class="row-inner">
							...
							<div class="col-md-4 col-sm-6 col-xs-12 selectsearchoptions">
								<?php
									...
									'class' => "form-control whiteBorder selectpicker online",
						...
						<div class="row-block settings-section">	
							...
							<div class="col-md-4 col-sm-6 col-xs-12">
								...
								<?php
									...
									'class' => "form-control imageBrowser online",
									]])->fileInput()->label('Upload Your Primary Profile Image<span> * </span>');
								?>
								<?php 
								$user_profile_images_new	=	Yii::$app->commonmethod->getUserProfilepics($attributes['id']);
								
								if(isset($user_profile_images_new['0']['profile_image']) && !empty($user_profile_images_new['0']['profile_image'])) { 
									$profile_pic = PROFILE_IMAGE_PATH . $user_profile_images_new['0']['profile_image'];?>
									<img class="img-responsive" id="profile_mod_a" src="<?php echo $profile_pic; ?>" alt="" width="50" height="50" style="position: relative;top: -40px;">
								<?php } ?>
							</div>	
							...
							<?php
								...
								?>
								<?php 
								$additional_images	=	Yii::$app->commonmethod->getUserAdditionalImage($attributes['id']);
								
								
								
									if(isset($additional_images) && !empty($additional_images)) {
										foreach($additional_images as $row_uimg) {
											$u_img = (isset($row_uimg['name']) ? UPLOAD_IMAGE.$row_uimg['name'] : '');
											if($u_img!='') { ?>
											
											<img class="img-responsive" id="upload_home_images_mod_d" src="<?php echo $u_img; ?>" alt="" width="50" height="50" style="position: relative;top: -35px;display:inline;vertical-align: top;"> 
											<?php
											}		
										}
									}	
							...
							<div class="row-block">
								<div class="col-md-6 col-sm-6 col-xs-12">
									...
									if(isset($Identity_documents) && !empty($Identity_documents)) {
										foreach($Identity_documents as $u_doc) {
											echo '<p style="position: relative;top: -40px;"><a href="'.DOCUMENT_DOWNLOAD_PATH.$u_doc['name'].'" target="_blank" id="upload_documents_mod_b"><i class="fa fa-paperclip" aria-hidden="true"></i>' .(isset($u_doc['name']) ? $u_doc['name'] : ''). '</a></p>';
										}
									}
									?>
								</div>	
								...
								<!--input id="uploadFile" placeholder="Add files from My Computer"/-->
									...
									<?php
										...
										'class' => "form-control imageBrowser online",
										]])->fileInput(['multiple' => true,"accept"=>"image/*"])->label('Upload 2 or more images of the inside of your home.<span> * </span><br><strong class="reduceSize">(Rest assured these pictures will not be displayed to the owners)</strong>');
									$user_home_images	=	Yii::$app->commonmethod->getUserDocuments($attributes['id'],HOME_IMAGES);
									if(isset($user_home_images) && !empty($user_home_images)) {
										foreach($user_home_images as $row_uimg) {
											$u_img = (isset($row_uimg['name']) ? UPLOAD_IMAGE.$row_uimg['name'] : '');
											if($u_img!='') { ?>
						...
						<div class="row-inner">
							...
							<div class="col-md-6 col-sm-6 col-xs-12">
								<?php
									...
									'class' => "form-control whiteBorder selctpettype online",
							...
							<div class="col-md-6 col-sm-6 col-xs-12 selectsearchoptions weightclass" id="weightid<?php echo $chkminservice ?>">								
								...
								'class' => 'form-control whiteBorder selectpicker online limit['.$chkminservice.']',
								'data-live-search' => "true",
								'multiple' => 'multiple',
						...
						setpetweight(weight,key);
								...
								<div class="col-md-6 col-sm-6 col-xs-12">
									<?php
										...
										'class' => "form-control whiteBorder lblchang1 online",
					...
					?>
						...
						<div class="row-inner">
							...
							<div class="col-md-6 col-sm-6 col-xs-12 selectsearchoptions weightclass" id="weightid<?php echo $j; ?>">								
								...
								'class' => 'form-control whiteBorder selectpicker online limit['.$j.']',
								'data-live-search' => "true",
								'multiple' => 'multiple',
								]])->dropDownList($pet_weight_limit, ['options' => $weightoptions])->label('Choose the sizes of pet you would accept <span class="required">*</span>');
								?>						               
					...
					$('#weightid'+key).hide();
						...
						<div id="addmoreblock">
							...
							<div class="col-md-6 col-sm-6 col-xs-12 selectsearchoptions weightclass">								
								...
								'class' => 'form-control whiteBorder selectpicker online limit0',
								'data-live-search' => "true",
								'multiple' => 'multiple',
								]])->dropDownList($pet_weight_limit, ['options' => $weightoptions])->label('Choose the sizes of pet you would accept <span class="required">*</span>');
								?>						               
								...
								<div class="col-md-4 col-sm-6 col-xs-12">
									<?php
										...
										'class' => "form-control whiteBorder online",
										]])->textInput(['maxlength'=>15 ,'type' => 'number', 'min' => 1, 'value'=>10,'autofocus' => true])->label('Price <span>*</span> <span id="lbl1"></span>');
									?>
								</div>	
								<!--div class="col-md-4 col-sm-6 col-xs-12">
						...
						<div class="row-block spasaller">
							...
							<div class="col-lg-11 col-md-10 col-sm-10 col-xs-8 btn-textinfo">Add another Type of Pet or Service</div>
							</div>
						...
						<div class="col-md-12 col-sm-12 col-xs-12">																	
									...
									if($status > 0){
												...
												echo Html::submitButton(Yii::t('yii','SUBMIT FOR APPROVAL'), ['class' => 'orangeBtn', 'name' => 'editUser-submit', 'id' => 'editUser-submit1']); 
									}else{
												echo Html::submitButton(Yii::t('yii','Save'), ['class' => 'orangeBtn', 'name' => 'editUser-submit', 'id' => 'editUser-submit']); 
												echo Html::submitButton(Yii::t('yii','SUBMIT FOR APPROVAL'), ['class' => 'orangeBtn-disable','disabled' => 'disabled','name' => 'editUser-submit', 'id' => 'editUser-submit1']); 
...
<script>
	$(document).ready(function() {
		$("select.lblchang1").children().first().remove();
	...
	$("#updatesitter-pet_weight_limit-0").on('change', function()			
	{ 
		var fld1 = fld2 = fld3 = fld4 = fld5 = fld6 = fld7 = fld8 = fld9 = fld10 = fld11 = fld12 = fld13 = fld14 = fld15 = 0;
		$(":text,number,:file,select,textarea").each(function() 
		{
		...
		$('#editUser-submit1').removeClass('orangeBtn');	
			...
			$(":text,:file,select,number,textarea").change(function ()
			{
				...
				$(":text,number,:file,select,textarea").each(function() 
				{
					...
					if ($(this).attr("name") == 'UpdateSitter[pet_weight_limit]['+newname+'][]') 
					{
						if($('.selectpicker').selectpicker('val')!=null) {
							fld14 = 0;
						}
			...
			$("#updatesitter-pet_weight_limit-"+newname).on('change', function()			
			{ 
    ...
    $.ajax({
                ...
                success: function (json) {
                    ...
                    for (var i = 0; i < json.length; i++) {
                        $("<option/*>").attr("value", json[i].id).text(json[i].cityAlias).appendTo($("#<?= $modelID ?>-city"));
                    }
