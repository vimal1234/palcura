...
$sitter_id=$booking_data['book_sitter_id'];
...
if (!empty($searchrequestdata)) {
    ...
    $model->number_of_pets=$searchrequestdata['selected_pal'];
    $model->services=$searchrequestdata['service_type'];
...
if(!empty($oldPalDetails))
{
    $oldPalDetails = $oldPalDetails;
}
else
{
    $oldPalDetails = '';
                    ...
                    <div class="formContent" style="margin:0;"> 
						...
						$('html, body').animate({
                            ...
                            <div class="row-block">
                                ...
                                <div class="col-md-6 col-sm-6 col-xs-12">
                                <span class=" field-booking-name">
                                <div class="form-group">

                                    <label for="booking-name"><p class="bookingnme">&nbsp </p>Price *
                                    
                                    </label>
                                        <input  class= "form-control", type="text" name="price" id="subtotal"  value="" readonly>
                                    </div>
                                </div>
                                </span>
                          </div>						
                            ...
                            <div class="row-block">
                                <div class="col-md-6 col-sm-6 col-xs-12 selectsearchoptions">
                                 ...
                                 if(isset($oldNoofPets)&& !empty($oldNoofPets)){ 
...
<script>
    $('document').ready(function () {
       ...
       /* Calculate final price using ajax*/
        getprice();

        function getprice(){

            var searchservice_type = $("#servicetype").val();
            var pets=$("#booking-number_of_pets").val();
            var from_date = new Date( $("#booking_from_date").val());
            var to_date =  new Date($('#booking_to_date').val()); 
            /*Calculate Days */
         days = (to_date- from_date) / (1000 * 60 * 60 * 24);
         days=Math.round(days);
       
         var querydata = 'sitterId=<?php echo $sitter_id; ?>&numOfDays='+days+'&numofPets='+pets+'&services='+searchservice_type+'&loggedingusertype=<?php echo $loggedingusertype; ?>';

                 
                 $.ajax({
                    type: "POST",
                    url:  '<?php echo SITE_URL; ?>'+'bookings/calculateprice',
                    data: querydata,
